...
def get_macro_real() -> Tuple[Dict, Dict]:
    """Carrega indicadores macroeconômicos com retry e diagnóstico"""
    kpis = {}
    historico = {}
    erros_detalhados = []
    ...
    for nome, codigo in APIConfig.SERIES_MACRO.items():
        ...
        # Tenta 3 URLs diferentes (a API do BCB mudou recentemente)
        urls_tentar = [
            f"https://api.bcb.gov.br/dados/serie/bcdata.sgs.{codigo}/dados/ultimos/60?formato=json",
            f"https://api.bcb.gov.br/dados/serie/bcdata.sgs.{codigo}/dados?formato=json&dataInicial=01/01/2020",
            f"http://api.bcb.gov.br/dados/serie/bcdata.sgs.{codigo}/dados/ultimos/60?formato=json"
        ]
        
        for tentativa, url in enumerate(urls_tentar, 1):
            try:
                resp = requests.get(
                    url, 
                    headers=APIConfig.HEADERS, 
                ...
                if resp.status_code != 200:
                    erros_detalhados.append(f"{nome}: HTTP {resp.status_code} (tentativa {tentativa})")
                    continue
                
                dados = resp.json()
                
                if not dados or not isinstance(dados, list) or len(dados) == 0:
                    erros_detalhados.append(f"{nome}: JSON vazio (tentativa {tentativa})")
                    continue
                ...
                if 'data' not in df.columns or 'valor' not in df.columns:
                    erros_detalhados.append(f"{nome}: Estrutura JSON inválida")
                    continue
                ...
                # Remove NaN
                df = df.dropna(subset=['valor', 'data_dt'])
                
                if df.empty:
                    erros_detalhados.append(f"{nome}: Dados válidos não encontrados")
                    continue
                
                # Limita aos últimos 60 registros se veio muito dado
                df = df.sort_values('data_dt', ascending=False).head(60)
                ...
                # KPI
                ultimo = df.iloc[0]  # Já está ordenado desc
                ...
                sucesso = True
                break  # Conseguiu! Sai do loop de URLs
                
            except requests.exceptions.Timeout:
                erros_detalhados.append(f"{nome}: Timeout na tentativa {tentativa}")
                time.sleep(0.5)
                continue
            except requests.exceptions.ConnectionError:
                erros_detalhados.append(f"{nome}: Erro de conexão (tentativa {tentativa})")
                time.sleep(0.5)
                continue
            except Exception as e:
                erros_detalhados.append(f"{nome}: {str(e)[:100]} (tentativa {tentativa})")
                time.sleep(0.5)
                continue
        
        if not sucesso:
            # Adiciona dados mockados para demonstração (remover em produção)
            pass
    
    # Se NENHUM indicador foi carregado, exibe diagnóstico
    if not kpis:
        st.error("### ⚠️ Diagnóstico de Erro - API BCB")
        with st.expander("🔍 Detalhes Técnicos dos Erros", expanded=True):
            st.markdown("**Possíveis causas:**")
            st.markdown("""
            1. 🔒 **Certificado SSL do BCB expirado/inválido**
            2. 🚫 **API temporariamente fora do ar** (manutenção)
            3. 🔄 **Mudança na estrutura da API** (nova versão)
            4. 🌐 **Problema de conectividade/firewall**
            5. ⏰ **Rate limiting** (muitas requisições)
            """)
            
            st.markdown("**Erros capturados:**")
            for erro in erros_detalhados[:10]:  # Mostra só os 10 primeiros
                st.code(erro)
            
            st.markdown("**🔧 Solução Temporária:**")
            st.info("Os dados macroeconômicos estão temporariamente indisponíveis. "
                   "O restante do dashboard (IPCA, câmbio, Focus) continua funcionando normalmente.")
