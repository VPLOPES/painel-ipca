...
def get_focus_data():
    try:
        ...
        # Ordenação rigorosa e remoção de duplicatas
...
# --- CÂMBIO (EXPANDER - SOLICITAÇÃO ATENDIDA) ---
with st.expander("💸 **Histórico de Câmbio (Dólar e Euro)**", expanded=False):
    df_moedas = get_currency_realtime()
    df_hist_cambio = get_cambio_historico()

    # KPIs
    dolar_bid, dolar_var = 0, 0
    euro_bid, euro_var = 0, 0
    dolar_ytd, euro_ytd = 0, 0
    ...
    if not df_hist_cambio.empty:
        ano_c_atual = datetime.now().year
        df_ytd = df_hist_cambio[df_hist_cambio.index.year == ano_c_atual]
        if not df_ytd.empty:
            dolar_ytd = ((df_ytd['Dólar'].iloc[-1] - df_ytd['Dólar'].iloc[0]) / df_ytd['Dólar'].iloc[0]) * 100
            euro_ytd = ((df_ytd['Euro'].iloc[-1] - df_ytd['Euro'].iloc[0]) / df_ytd['Euro'].iloc[0]) * 100
    ...
    ctab1, ctab2, ctab3 = st.tabs(["📈 Histórico", "🗓️ Matriz", "📋 Tabela"])
    ...
    with ctab2:
        if not df_hist_cambio.empty:
            moeda_matriz = st.radio("Moeda:", ["Dólar", "Euro"], horizontal=True)
            ...
            except: st.error("Matriz indisponível.")
